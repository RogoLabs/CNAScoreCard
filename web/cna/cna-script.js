// CNA Detail Page Script - Displays pre-calculated EAS scores
let cnaData = {};
let cveScores = [];

// Improved extraction of CNA name from URL or query string
function getCNANameFromURL() {
    // Try query string first
    const params = new URLSearchParams(window.location.search);
    if (params.has('cna')) {
        return params.get('cna');
    }
    // Otherwise, use last non-empty, non-generic part of the path
    const pathParts = window.location.pathname.split('/').filter(Boolean);
    // Remove generic directory names
    const blacklist = ['cna', 'data', 'web'];
...
window.CNA_NAME = getCNANameFromURL();
window.SAFE_FILENAME = window.CNA_NAME.replace(/[^a-zA-Z0-9_-]/g, '_');
